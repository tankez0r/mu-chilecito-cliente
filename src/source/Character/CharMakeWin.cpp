...
void CCharMakeWin::RenderCreateCharacter()
{
    ...
    EndOpengl();
    // Restore the 2D ortho projection (and disable depth test) that the rest of
    // RenderControls()'s sprite/button drawing needs -- BeginOpengl() above left GlobalUBO
    // on this panel's 3D perspective camera, same idiom as every other embedded 3D preview
    // panel (see PersonalShopTitleImp.cpp, NewUI3DRenderMng.cpp's CNewUI3DCamera::Render()).
    BeginBitmap();
}